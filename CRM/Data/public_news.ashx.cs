    ...
    public class public_news : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (request["Action"] == "save")
            {
                ...
                model.news_content = PageValidate.InputText(request["T_content"], 40000);
            ...
            if (request["Action"] == "grid")
            {
                ...
                if (!string.IsNullOrEmpty(request["sstart"]))
                    serchtxt += " and news_time >= '" + request["sstart"] + "'";
                ...
                if (!string.IsNullOrEmpty(request["stext"]))
                {
                    if (request["stext"] != "输入关键词搜索")
                        serchtxt += " and news_title like N'%" + request["stext"] + "%'";
                }
            ...
            if (request["Action"] == "del")
            {
                ...
                if (dsemp.Tables[0].Rows.Count > 0)
                {
                    ...
                    else
                    {
                        ...
                        string delauth = getauth.GetBtnAuthority(request.Cookies["UserID"].Value, "10");
                        if (delauth == "F")
                            canDel = false;
                        else
                            canDel = true;
